...
return mob.init{
    zapProc = function(self, enemy, dmg)
        ...
        if newMove >= 4 and newMove <= 6 then
            if enemy:buffLevel("Necrotism") > 0 then
                RPD.prolongBuff(enemy, RPD.Buffs.Necrotism, math.random(6*mul,14))
                if math.random(1,100) <= 25 then
                    if enemy:buffLevel("Nightmare") > 0 then
                        RPD.prolongBuff(enemy, "Nightmare", math.random(6*mul,14))
                    else
                        RPD.affectBuff(enemy, "Nightmare", 5*mul)
                    end
                end
            else
                RPD.affectBuff(enemy, RPD.Buffs.Necrotism, 5*mul)
            end
            return dmg*mul/res
                ...
                for j = -3*mul, 3*mul do
                    if i ~= 0 and j ~= 0 then
                        ...
                        if level:cellValid(newpos) and validTiles[level.map[newpos]] and not target then
                            self:getSprite():emitter():burst(RPD.Sfx.ShadowParticle.CURSE, 6/mul)
            ...
            for i = -3*mul, 3*mul do
                for j = -3*mul, 3*mul do
                    if i ~= 0 or j ~= 0 then
                        ...
                        local target = RPD.Actor:findChar(newpos)
                        if level:cellValid(newpos) and validTiles[level.map[newpos]] and not target then
                            enemy:getSprite():emitter():burst(RPD.Sfx.ShadowParticle.CURSE, 6/mul)
    ...
    attackProc = function(self, enemy, dmg)
        ...
        enemy:getSprite():emitter():burst(RPD.Sfx.ShadowParticle.UP, 6/mul)
    ...
    stats = function(self)
        ...
        self:resistances():add(RPD.Enchantments.Death)
        ...
        self:resistances():add("DeathlyRecruit")
        self:resistances():add("Nightmare")
        self:resistances():add(RPD.Glyphs.Displacement)
        self:resistances():add(RPD.ItemFactory:itemByName("WandOfShadowbolt"))
    end
}