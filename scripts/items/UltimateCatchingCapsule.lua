...
return item.init{
    desc  = function ()
        return {
            ...
            name      = "UltimateCatchingCapsule_Name",
            info      = "UltimateCatchingCapsule_Desc",
            stackable = false,
            upgradable= false,
            price     = 0
        }
    ...
    onThrow = function(self, item, cell)
        ...
        if self.data.CapsuleMon.ins then
            ...
            RPD.glogp("Capsule_Nothing",RPD.textById(pref))
            hero:collect(item)
        end
    end
end,
        ...
        if action == "Release_Mob" then
            if self.data.CapsuleMon.class then
                if self.data.CapsuleMon.ins then
                    ...
                    if self.data.CapsuleMon.class--[[:getEntityKind()]] == "BlackCat" or self.data.CapsuleMon.class == "CockerSpaniel" or self.data.CapsuleMon.class == "CockerSpanielNPC" then
